...
def get(data_path, task_order, seed=0, pc_valid=0.05, flat=False):
    ...
    for idx in task_order:
        data[idx] = {}
        data[idx]['name'] = 'cifar100-' + str(idx)
        data[idx]['ncla'] = 5
    ...
    if not os.path.isdir(data_path):
        ...
        dataset_train = datasets.CIFAR100(data_path, train=True, download=True)
        dataset_test = datasets.CIFAR100(data_path, train=False, download=True)

    for s in ['train', 'test']:
        if s == 'train':
            path = os.path.join(data_path, 'cifar-100-python/train')
        else:
            path = os.path.join(data_path, 'cifar-100-python/test')
        ...
        for idx in task_order:
            ...
            data[idx][s]['x'] = torch.FloatTensor(gimages)
            #data[idx][s]['y'] = torch.LongTensor(np.array([np.int32(glabels)], dtype=int)).view(-1)
            data[idx][s]['y'] = torch.LongTensor(np.array([np.int32(glabels) + idx*5], dtype=int)).view(-1)
            # print(data[idx][s]['x'].max(), data[idx][s]['x'].min())

            if s == 'train':
                r = np.arange(data[idx][s]['x'].size(0))
